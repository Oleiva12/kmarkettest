...
var browser = require('ollama/browser');
...
class Ollama extends llms.ToolCallLLM {
    static{
    ...
    constructor(params){
        super(), this.supportToolCall = (_initProto(this), true), this.options = {
            num_ctx: 4096,
            top_p: 0.9,
            temperature: 0.7
        };
        this.model = params.model;
        this.ollama = new browser.Ollama(params.config);
        if (params.options) {
            this.options = {
                ...this.options,
                ...params.options
    ...
    async chat(params) {
        ...
        if (responseFormat && this.metadata.structuredOutput) {
            if (zod.isZodSchema(responseFormat)) payload.format = zod.zodToJsonSchema(responseFormat);
        }
        if (!stream) {
            ...
            return utils.streamConverter(stream, messageAccessor);
        }
    }
    async complete(params) {
        ...
        const payload = {
            model: this.model,
            prompt: utils.extractText(prompt),
            stream: !!stream,
            options: {
                ...this.options
            }
        ...
        if (!stream) {
            ...
            return utils.streamConverter(stream, completionAccessor);
        }
    }
    static toTool(tool) {
        return {
 ...
 */ const ollama = (init)=>new Ollama(init);

class OllamaAgentWorker extends agent.LLMAgentWorker {
}
class OllamaAgent extends agent.LLMAgent {
    constructor(params){
        const llm = params.llm ?? (global.Settings.llm instanceof Ollama ? global.Settings.llm : !params.model ? (()=>{
...
class OllamaEmbedding extends embeddings.BaseEmbedding {
    constructor(params){
        super();
...
exports.OllamaAgentWorker = OllamaAgentWorker;
